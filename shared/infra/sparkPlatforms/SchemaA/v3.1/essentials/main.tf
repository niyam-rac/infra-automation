...
module "faStorageAccountModules" {
  source = "../../../../modules/storage/v1.0"

  resourceGroupName               = data.azurerm_resource_group.projectResourceGroup.name
  location                        = data.azurerm_resource_group.projectResourceGroup.location
  tags                            = var.tags
  storageAccountName              = var.faStorageAccount.name
  infrastructureEncryptionEnabled = var.faStorageAccount.infrastructureEncryptionEnabled
  publicNetworkAccess             = var.faStorageAccount.publicNetworkAccess
}
...
module "dataStorageAccountModules" {
  ...
  infrastructureEncryptionEnabled = var.dataStorageAccount.infrastructureEncryptionEnabled
}

module "keyvaultModules" {
  source = "../../../../modules/keyvault/v1.1"
