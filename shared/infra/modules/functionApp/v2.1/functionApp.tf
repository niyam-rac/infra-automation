...
locals {
  ...
  ipRestrictionDefaultAction     = length(var.ipRestrictions) > 0 ? "Deny" : "Allow"
}
...
resource "azurerm_linux_function_app" "functionApp" {
  ...
  site_config {
    ...
    ip_restriction_default_action          = local.ipRestrictionDefaultAction

    dynamic "ip_restriction" {
      for_each = var.ipRestrictions
      content {
